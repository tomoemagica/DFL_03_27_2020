...
def sort_by_saturation(input_path):
    io.log_info ("Sorting by saturation...")
    img_list = [ [x, np.mean ( cv2.cvtColor(cv2_imread(x), cv2.COLOR_BGR2HSV)[...,1].flatten()  )] for x in io.progress_bar_generator( pathex.get_image_paths(input_path), "Loading") ]
    io.log_info ("Sorting...")
    img_list = sorted(img_list, key=operator.itemgetter(1), reverse=True)
    return img_list, []
...
sort_func_methods = {
    ...
    'hue':         ("hue", sort_by_hue),
    'saturation':  ("saturation", sort_by_saturation),
